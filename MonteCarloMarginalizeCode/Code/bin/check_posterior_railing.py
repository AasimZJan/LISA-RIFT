...
parser.add_argument("--cip-file", type=str, help="filname of the CIP *.sub file")
parser.add_argument("--posterior-file", type=str, help="filname of the posterior *.dat file")
parser.add_argument("--puff-factor", type=float, default=0.2, help="Puff factor for the updated bound")
parser.add_argument("--p", type=float, default=0.1, help="Probability in which we consider railing")
parser.add_argument("--parameter", action='append', help="List of parameters that we want to check the railing of")
...
def check_bounds(samples, p=0.1, puff_factor=0.2, forced_bounds=[-np.inf,np.inf]):
    """
    ...
    Args:
        ...
        p (float, optional): Threshold probability for detecting railing conditions. Default is 0.1.
...
#Load the CIP file and get the actual parameter ranges
old_bounds = load_cip(opts.cip_file)
print(f"Old bounds: {old_bounds}")
#Check the railing and obtain the bounds for the desired parameters
print(opts.parameter)
new_bounds = update_bounds(opts.posterior_file, opts.parameter)
print(f"New bounds: {new_bounds}")
#Change the CIP file
print(f"CIP file UPDATED")